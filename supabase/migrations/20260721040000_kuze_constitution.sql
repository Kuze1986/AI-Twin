-- Kuze Constitution — foundational governance layer.
-- The constitution is the top-level charter every system prompt is built on top of. Prior to
-- this it lived only as tag-filtered context blocks in kuze.identity_profile that the runtime
-- (which reads public.identity_profile) never loaded — i.e. Kuze was not actually governed by
-- it. This table makes it a first-class, versioned, always-injected layer.
--
-- Immutable by policy: the active row changes only by deliberate re-ratification (a new
-- version row + flipping is_active). RLS on, service-role-only, like the rest of the kuze schema.

CREATE SCHEMA IF NOT EXISTS kuze;

CREATE TABLE IF NOT EXISTS kuze.constitution (
...
-- At most one active constitution at a time.
CREATE UNIQUE INDEX IF NOT EXISTS constitution_one_active
  ON kuze.constitution (is_active) WHERE is_active;
CREATE UNIQUE INDEX IF NOT EXISTS constitution_version_uniq
  ON kuze.constitution (version);

ALTER TABLE kuze.constitution ENABLE ROW LEVEL SECURITY;

-- Seed V1 (verbatim from supabase/kuze-constitution-v1.md). Dollar-quoted so the body's
-- apostrophes need no escaping.
INSERT INTO kuze.constitution (version, title, body, is_active, ratified_at)
SELECT 1, 'Kuze Constitution V.1', $constitution$# Kuze Constitution V.1
...
— Brandon Alexander
BioLoop Holdings
$constitution$, true, now()
WHERE NOT EXISTS (SELECT 1 FROM kuze.constitution WHERE version = 1);
